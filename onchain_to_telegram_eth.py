...
def read_counter(counter_file: str, key: str, start_counter: int = 1) -> int:
    """
    Lê/atualiza contador por chave (ex.: eth_diario/eth_semanal/eth_mensal) em counters.json.
...
def build_prompt(data_str: str, numero: int, metrics: Optional[Dict[str, Any]], label: str) -> str:
    header = f"Dados On-Chain — ETH — {data_str} — {label} — Nº {numero}"
    rules = (
...
def main():
    load_env_if_present()
    ap = argparse.ArgumentParser(description="Relatório on-chain (ETH) → Telegram.")
    ...
    key_map   = {"daily": "eth_diario", "weekly": "eth_semanal", "monthly": "eth_mensal"}
    label     = label_map.get(args.period, "Diário")
    key       = key_map.get(args.period, "eth_diario")
    ...
    # Para ETH: tenta ETH_TELEGRAM_CHAT_ID; se vazio, usa TELEGRAM_CHAT_ID
    tg_chat  = os.environ.get("ETH_TELEGRAM_CHAT_ID") or os.environ.get("TELEGRAM_CHAT_ID") or ""
    if args.send_as in ("message","both") and (not tg_token or not tg_chat):
        raise SystemExit("Defina TELEGRAM_BOT_TOKEN e ETH_TELEGRAM_CHAT_ID (ou TELEGRAM_CHAT_ID) para envio por mensagem.")
    ...
    titulo = f"📊 <b>Dados On-Chain — ETH — {data_str} — {label} — Nº {numero}</b>"
    ...
    if args.send_as in ("pdf","both"):
        ...
        path = os.path.join(out_dir, f"Dados On-Chain — ETH — {data_str} — {label} — Nº {numero}.txt")
