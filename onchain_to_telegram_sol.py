...
ASSET = "SOL"
ASSET_LOWER = "sol"
SENT_DIR = pathlib.Path(".sent") / ASSET_LOWER
...
PROMPT_TEMPLATE = """Gere o relatório “Dados On-Chain SOL — {data_long} — {periodo_pt} — Nº {numero}” para {ativo}.
...
def _send_telegram(text: str, as_document: bool = False):
    ...
    chat_id = os.getenv("SOL_TELEGRAM_CHAT_ID") or os.environ.get("TELEGRAM_CHAT_ID")
    if not chat_id:
        raise RuntimeError("Defina SOL_TELEGRAM_CHAT_ID ou TELEGRAM_CHAT_ID")
    topic_id = os.getenv("SOL_TELEGRAM_TOPIC_ID")
